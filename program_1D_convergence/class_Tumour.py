...
class Tumour:
    ...
    epsilon = 0.02                    # interface parameter, epsilon > dt (!)
    beta    = 0.1                     # surface tension
    lam_P   = 20                      # proliferation rate
    lam_A   = DOLFIN_EPS                 # apoptosis rate
    lam_C   = 2.0                      # consumption rate
    ...
    dynamic = False                     # dynamic or quasi-static eq for sigma
    ...
    def init_problem(self, dt, u, u0):
        ...
        #f    = 0.25*(c+1)**2*(1-c)**2
        #DPsi = diff(f, c)
        DPsi = (c**3-c)        # derivative of potential, dpsi/dphi
        m_phi = 1.0 
        h_phi = 0.5*(1+c)       # interpolation function h(phi)
...
class InitialConditions(UserExpression):
    ...
    def eval(self, values, x):
        values[0] = function_1(x, model,0.1)
        if model.num_eq >=2:
            values[1] = 0.0
        if model.num_eq >= 3:
            values[2] = model.sig0
